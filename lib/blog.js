...
function loadEntry(root, entry, tz, cb) {
  ...
  fs.readFile(fpath, 'utf8', function(err, text) {
    ...
    if (doc.front.author) {
      doc.front.author = parseAuthor(doc.front.author);
    }
    if (doc.front.contributors) {
      for (i = 0, len = doc.front.contributors.length; i < len; ++i) {
        doc.front.contributors[i] = parseAuthor(doc.front.contributors[i]);
      }
    }
    
    if (doc.front.published instanceof Date) {
      doc.front.publishedAt = doc.front.published;
    } else if (typeof doc.front.published == 'string') {
      doc.front.publishedAt = moment.tz(doc.front.published, tz).toDate();
    }
    delete doc.front.published;
    if (doc.front.updated instanceof Date) {
      doc.front.updatedAt = doc.front.updated;
    } else if (typeof doc.front.updated == 'string') {
      doc.front.updatedAt = moment.tz(doc.front.updated, tz).toDate();
    }
    delete doc.front.updated;
    
    merge(entry, doc.front);
